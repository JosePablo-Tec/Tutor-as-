...
public void llenarTablero() {
    for (int i = 0; i < m.length; i++) {
        List<Integer> numerosEnColumna = new ArrayList<>();  // Mueve la inicialización aquí
        for (int j = 0; j < m[i].length; j++) {
            for (int k = 0; k < m[i][j].length; k++) {
                ...
                } while (buscarRepetidosArray(numeroGenerado, numerosEnColumna));
                m[i][j][k] = numeroGenerado;
                numerosEnColumna.add(numeroGenerado);
            }
        }
    }
}
   
    public boolean buscarRepetidosArray(int elem, List<Integer> numerosEnColumna){
        if(numerosEnColumna.contains(elem)){
            return true;
        }
        else{
            return false;
